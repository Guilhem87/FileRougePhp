...
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';// Import GLTF fichier using format JSON.
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';// Importer contrôl cam OrbitControls
...
camera.position.z= 1.3;// .z = 1.2 Cela permet de voir les objets dans la scène qui sont proches de l'origine.
camera.position.x= 0;
camera.position.y= 0;
...
const coudeZone = new THREE.SphereGeometry(0.01, 32, 32); // création d'une sphere avec la fonction shpereGeo
...
const coudeMesh = new THREE.Mesh(coudeZone, material); //on met dans la constante coudeMesh un nouvelle objet ds la scene 3D
// qui va associer la sphere et le material
...
//===FONCTION POUR CREER DES ZONES SUR LA SCENE=====//
...
const  quadri = createBodyPart('quadri', 0x00ff00, {x:-0.14, y:-0.32, z:0.116});
...
const molet = createBodyPart('molet', 0x00ff00, {x:-0.14, y:-0.50, z:-0.08});
...
// maj pour le raycaster en fonction de la caméra et de la position de la souris
    ...
    passeSurZoneetChangeColorTaille(raycaster, molet, 0xff0000, 0x00ff00);
});

//cf https://threejs.org/examples/?q=poin#webgl_interactive_points (changement zone taille)
//FONCTION POUR AGRANDIR LA ZONE ET LA CHANGER DE COULEUR
...
envoyerVersLien(genoux, './controller_pages.php');
envoyerVersLien(coudeMesh, './controller_blessure.php?id_zone_blessure=3');
envoyerVersLien(trapeze, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(basDos, './controller_blessure.php?id_zone_blessure=2');
envoyerVersLien(fesse, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(epaule, './controller_blessure.php?id_zone_blessure=1');
envoyerVersLien(flechiHanche, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(abdos, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(cheville, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(quadri, './controller_blessure.php?id_zone_blessure=4');
envoyerVersLien(nuqueTrap, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(voutePied, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(inserbiceps, './controller_blessure.php?id_zone_blessure=');
envoyerVersLien(molet, './controller_blessure.php?id_zone_blessure=5');
